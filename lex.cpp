...
public:
  ...
  virtual void ShowAST(int level){
    fprintf(stderr, "%*cExpr\n", level, ' ');
  };
};
...
public:
  ...
  void ShowAST(int level) override{
    fprintf(stderr, "%*cVariableExprAST(%s)\n", level, ' ', Name.empty() ? "" : Name.c_str());
  }
};
...
public:
  ...
  void ShowAST(int level) override{
    fprintf(stderr, "%*cUnaryExprAST(%c)\n", level, ' ', Opcode);
    Operand->ShowAST(level + 1);
  };
};
...
public:
  ...
  void ShowAST(int level) override{
    fprintf(stderr, "%*cBinaryExprAST(%c)\n", level, ' ', Op);
    LHS->ShowAST(level + 1);
    RHS->ShowAST(level + 1);
  };
};
...
public:
  ...
  void ShowAST(int level){
    fprintf(stderr, "%*cFunction\n", level, ' ');
    // Proto->ShowAST(level + 1);
    Body->ShowAST(level + 1);
  };
};
...
public:
  ...
  void ShowAST(int level)override{
    fprintf(stderr, "%*cIfExprAST\n", level, ' ');
    Cond->ShowAST(level + 1);
    Then->ShowAST(level + 1);
    Else->ShowAST(level + 1);
  }
};
...
public:
  ...
  void ShowAST(int level)override{
    fprintf(stderr, "%*cForExprAST(%s)\n", level, ' ', VarName.c_str());
    Start->ShowAST(level + 1);
    End->ShowAST(level + 1);
    if(Step) Step->ShowAST(level + 1);
    Body->ShowAST(level + 1);
  }
};
...
static std::unique_ptr<PrototypeAST> ParsePrototype() {
  ...
  return std::make_unique<PrototypeAST>(FnLoc, FnName, std::move(ArgNames), Kind != 0,
                                         BinaryPrecedence);
}
...
static std::unique_ptr<FunctionAST> ParseTopLevelExpr() {
  ...
  if (auto E = ParseExpression()) {
    ...
    auto Proto = std::make_unique<PrototypeAST>(FnLoc, outputObjectFile && defineMain ? "main" : "__anon_expr", std::vector<std::string>());
    return std::make_unique<FunctionAST>(std::move(Proto), std::move(E));
  }
  return nullptr;
}
...
Function *getFunction(std::string Name) {
  ...
  std::cerr << "Function " << Name << " not found in this module, creating..\n";

  // If not, check whether we can codegen the declaration from some existing
  // prototype.
  auto FI = FunctionProtos.find(Name);
  if (FI != FunctionProtos.end()){
    std::cerr << "Function " << Name << " found in prototypes, codegening..\n";
...
Value *CallExprAST::codegen() {
  ...
  if (!CalleeF)
    return LogErrorV((std::string("Unknown function ") + Callee + " referenced").c_str());
...
Function *FunctionAST::codegen() {
  ...
  std::cerr << "\nDumping prototype table: \n";
  for(auto& proto : FunctionProtos)
    std::cerr << "  proto " << proto.first << "\n";
...
static void HandleTopLevelExpression() {
  ...
  if (auto expr = ParseTopLevelExpr()) {
    ...
    if (!outputObjectFile && FnIR) {
      ...
      auto ExprSymbol = TheJIT->findSymbol(outputObjectFile && defineMain ? "main" : "__anon_expr");
